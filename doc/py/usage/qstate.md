...
また、複数のメソッドを同時に登録したい場合は、クラス・メソッド'add_methods'を使って、以下のようにします。引数として設定できる関数の数には特に制限はありません。
...
C[00..0]の係数が正の実数になるようにしています。C[00..0]が0の場合は、
位相項を括りだすことはしません。
...
get_ampメソッドに関する注意事項を一言。指定した量子ビットとそれ以外が
エンタングルしている場合、実行のたびに結果が変わります。純粋状態の部分
系は一般に混合状態なので。
...
果が変わりますのでご注意ください。純粋状態の部分系は一般に混合状態なので。
...
びに結果が変わりますのでご注意ください。純粋状態の部分系は一般に混合状
態なので。
...
例えば、以下のようにやればできます。
    ...
    for _ in range(100):
        ...
        qs.x(1).m(qid=[1], tag='foo')
        freq += qs.m_freq(tag='foo')
        qs.reset()
    print(freq)

tag引数とかm_freqメソッドについては、少し下の「測定の実行と測定データ
の取得（もう一つの方法）」に説明がありますので、そちらを見てください。
...
最後の結果は、測定値を十進数に変換した値です。

Z軸方向以外の測定の場合、u=>0,d=>1のように読み替えて、十進数にしていま
す。ベル測定の場合は、phi+ => 0, phi- => 3, psi+ => 1, psi- => 2と定義
しています。
...
シュミット係数だけを取得したい場合は、schmidt_coefメソッドを使います。
    ...
    coef = qs.schmidt_decomp(qid_0=[0,1], qid_1=[2,3,4])
