...
class QStateLogical(QState):
    ...
    def set_register(self, dat_left, dat_rignt, bnd, anc):
        ...
        self.__lz = {'left': PauliProduct(pauli_str='ZZ', qid=[dat_left[0], dat_left[1]]),
                     'right': PauliProduct(pauli_str='ZZ', qid=[dat_right[0], dat_right[1]]),
                     'whole': PauliProduct(pauli_str='ZZZZ', qid=[dat_left[0], dat_left[1], dat_right[0], dat_right[1]])}

        return self

    def initialize(self, alpha=0.0, beta=0.0, gamma=0.0):
        ''' inject random quantum state to logical qubit of left site  (|left> = a |0> + b |1>, |right> = |0>) '''
    ...
    def merge(self):
        '''  set |0> to boundary qubits and measure the X stabilizers including boundary qubits '''

        parity = 1
        self.reset(qid=self.__bnd)
        for x_stab in self.__x_stab['bnd']:
            self.reset(qid=[self.__anc[0]])
            self.h(self.__anc[0])
            self.operate(ctrl=self.__anc[0], pp=x_stab)
            self.h(self.__anc[0])
    ...
    def Mz(self, shots=1):
        ...
        self.reset(qid=[self.__anc[0]])
        self.h(self.__anc[0])
        self.operate(ctrl=self.__anc[0], pp=self.__lz['whole'])
        self.h(self.__anc[0])
