...
from qlazy import QCirc, Backend, PauliProduct, MPState
...
def equal_mpstates(mps_0, mps_1, qid=None):

    if qid is None:
        fid = mps_0.fidelity(mps_1)
    else:
        fid = mps_0.fidelity(mps_1, qid=qid)
...
class TestBackend_1_qubit_qlazy_mps_simulator(unittest.TestCase):
    """ test 'Backend' : 1-qubit gate (qlazy_mps_simulator)
    """
...
class TestBackend_measure_qstate_simulator(unittest.TestCase):
    ...
    def test_measure_mesurement_unitary_measurement(self):
        ...
        self.assertEqual(list(mpstate.amp), [0.+0.j, 0.+0.j, 1.+0.j, 0.+0.j])
...
class TestBackend_random_mps_simulator(unittest.TestCase):
    """ test 'Backend' : execute random circuit
    """

    def test_random_1(self):
        """test 'random'
        """
        bk_mps = Backend(product='qlazy', device='mps_simulator')
        bk_qs = Backend(product='qlazy', device='qstate_simulator')
        qc = QCirc.generate_random_gates(qubit_num=10, gate_num=50, phase=(0.0, 0.25, 0.5),
                                         prob={'x': 3, 'z': 3, 'h':2, 'cx':1, 'rx':1, 'crz':1})
        res = bk_mps.run(qcirc=qc, out_state=True)
        vec_mps = res.mpstate.amp
        res = bk_qs.run(qcirc=qc, out_state=True)
        vec_qs = res.qstate.amp
...
class TestBackend_init_option_qstate_simulator(unittest.TestCase):
    ...
    def test_init_option_run_2(self):
        ...
        qc = qc_A + qc_B
        mps_expect = bk.run(qcirc=qc, out_state=True).mpstate
        mps_ini = MPState(qubit_num=4).h(0).h(1).crx(0, 2, phase=0.7).crz(1,2, phase=0.3)
        mps_actual = bk.run(init=mps_ini, qcirc=qc_B, out_state=True).mpstate
        self.assertEqual(equal_mpstates(mps_expect, mps_actual, qid=[0,1,2]), True)
