...
class TestQComp_operate_qlazy_qstate_simulator(unittest.TestCase):
    """ test 'QComp' : operate
    """
    ...
    def test_operate_x(self):
        """test 'operate' (x)
        """
        qc_expect = QComp(product='qlazy', device='qstate_simulator').x(0)
        res = qc_expect.run()
        expect = res.info['qstate'].amp
        pp = PauliProduct(pauli_str="X")
        qc_actual = QComp(product='qlazy', device='qstate_simulator').operate(pp=pp)
        res = qc_actual.run()
        actual = res.info['qstate'].amp
        ans = equal_vectors(actual, expect)
        self.assertEqual(ans,True)
    ...
    def test_operate_h_x(self):
        ...
        """
        qc_expect = QComp(product='qlazy', device='qstate_simulator').h(0).x(0)
        res = qc_expect.run()
        expect = res.info['qstate'].amp
        pp = PauliProduct(pauli_str="X")
        qc_actual = QComp(product='qlazy', device='qstate_simulator').h(0).operate(pp=pp)
        res = qc_actual.run()
        actual = res.info['qstate'].amp
        ans = equal_vectors(actual, expect)
        self.assertEqual(ans,True)
    ...
    def test_operate_h_y(self):
        ...
        """
        qc_expect = QComp(product='qlazy', device='qstate_simulator').h(0).y(0)
        res = qc_expect.run()
        expect = res.info['qstate'].amp
    ...
    def test_operate_h_z(self):
        ...
        """
        qc_expect = QComp(product='qlazy', device='qstate_simulator').h(0).z(0)
        res = qc_expect.run()
        expect = res.info['qstate'].amp
    ...
    def test_operate_xyz(self):
        """test 'operate' (xyz)
        """
        qc_expect = QComp(product='qlazy', device='qstate_simulator').x(2).y(0).z(1)
        res = qc_expect.run()
        expect = res.info['qstate'].amp
    ...
    def test_operate_controlled_xyz(self):
        ...
        """
        qc_expect = QComp(product='qlazy', device='qstate_simulator').cx(3,2).cy(3,0).cz(3,1)
        res = qc_expect.run()
        expect = res.info['qstate'].amp
        pp = PauliProduct(pauli_str="XYZ", qid=[2,0,1])
        qc_actual = QComp(product='qlazy', device='qstate_simulator').operate(pp=pp, ctrl=3)
        res = qc_actual.run()
        actual = res.info['qstate'].amp
        ans = equal_vectors(actual, expect)
        self.assertEqual(ans,True)
...
class TestQComp_reset_qstate_simulator(unittest.TestCase):
...
class TestQComp_inheritance_qstate_simulator(unittest.TestCase):
    ...
    def test_inheritance(self):
        """test 'inheritance'
        """
        qc = MyQComp(product='qlazy', device='qstate_simulator')
